...
Primero que todo el proyecto consiste de 4 modelos diferentes todos ellos conectados entre si, aparte de esto aprovecho el modelo de usuario que ofrece Django para unirlo a otro de los modelos más inferiores en la escala.
...
ediciones son colecciones de cartas que van saliendo cada X meses trayendo cartas nuevas o algunas creadas anteriormente. Aparte de esto último, las cartas 
tambien pueden ser de multiples colores y de diferentes tipos aunque esta última no la incluí por no complicar demasiado los modelos. 
...
ya que sino lo añadiria a alguien que no existe. Junto a todo esto tambien durante el login si por algun motivo te equivocas el sistema te lo hara saber con mensajes de igual manera que cuando tye hayas deslogueado de manera existosa.
...
Para tu poder añadir cartas a tu colección debera de ser creadad anteriormente por el administrador del sistema. Una vez que esten en la tabla principal podras clicar a añadir y se añadira a tu colección. Aunque pulses multiples veces
solo se te añadira una vez, el resto de ellas hara que aumente el número de la misma carta en propiedad.
...
te da la posibilidad de cambiar el estado de la carta y la cantidad total que tienes de la misma, ojo el minimo siempre sera 1 para evitar errores.
...
Como reglas de negocio que elegi a petición del ejercicio ha sido la de evitar la repetición de la misma carta en la colección y la de evitar número en 0 o negativos. Como he explicado antes se evita tanto
en las vistas como en el input que puede introducir el usuario en las páginas web
el añadir la misma carta 2 veces. 
